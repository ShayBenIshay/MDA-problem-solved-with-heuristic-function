    ...
    def solve_problem(self, problem: GraphProblem) -> SearchResult:
        with Timer(print_title=False) as timer:
            ...
            while (high_heuristic_weight - low_heuristic_weight) > 0.01:
                ...
                mid_w_value = (high_heuristic_weight + low_heuristic_weight) / 2
                anytime_w_astar = AStar(heuristic_function_type=self.heuristic_function_type, heuristic_weight=mid_w_value,
                                         max_nr_states_to_expand=self.max_nr_states_to_expand_per_iteration)
                anytime_w_astar_res = anytime_w_astar.solve_problem(problem)

                total_nr_expanded_states += anytime_w_astar_res.nr_expanded_states
                max_nr_stored_states = max(max_nr_stored_states, anytime_w_astar_res.max_nr_stored_states)

                if anytime_w_astar_res.is_solution_found:
                    high_heuristic_weight = mid_w_value
                    if anytime_w_astar_res.solution_g_cost < best_solution.solution_g_cost:
                        best_solution = anytime_w_astar_res
                        best_heuristic_weight = mid_w_value
                else:
                    low_heuristic_weight = mid_w_value
