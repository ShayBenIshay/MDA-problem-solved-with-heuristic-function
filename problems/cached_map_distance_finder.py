...
class CachedMapDistanceFinder:
    ...
    def get_map_cost_between(self, src_junction: Junction, tgt_junction: Junction) -> Optional[Cost]:
        ...
        map_pro = MapProblem(self.streets_map, src_junction.index, tgt_junction.index)
        sol = self.map_problem_solver.solve_problem(map_pro)

        # no solution has found
        if not sol.is_solution_found:
            self._insert_to_cache(src_dst, None)
            return None

        self._insert_to_cache(src_dst, sol.solution_cost)
