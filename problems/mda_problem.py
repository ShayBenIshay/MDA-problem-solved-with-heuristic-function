...
class MDAProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        successors_set: List[MDAState] = []

        # goes through the reported apartments waiting to visit, and adds the ones which can be visited to the successors_set
        waiting_apartments = self.get_reported_apartments_waiting_to_visit(state_to_expand)
        for apar in waiting_apartments:
            if (((apar not in state_to_expand.tests_on_ambulance) and (apar not in state_to_expand.tests_transferred_to_lab))\
                    and (apar.nr_roommates <= state_to_expand.nr_matoshim_on_ambulance)\
                    and (apar.nr_roommates <= self.problem_input.ambulance.taken_tests_storage_capacity\
                         - state_to_expand.get_total_nr_tests_taken_and_stored_on_ambulance())):

                matoshim = state_to_expand.nr_matoshim_on_ambulance - apar.nr_roommates
                tests_on_ambulance = (state_to_expand.tests_on_ambulance | {apar})
                new_state = MDAState(apar, frozenset(tests_on_ambulance),\
                                     state_to_expand.tests_transferred_to_lab, matoshim, state_to_expand.visited_labs)
                successors_set.append(new_state)

        # goes through the labs, and adds the ones which can be visited to the successors_set
        for lab in self.problem_input.laboratories:
            if len(state_to_expand.tests_on_ambulance) or (lab not in state_to_expand.visited_labs):
                transferred = set(state_to_expand.tests_transferred_to_lab) | set(state_to_expand.tests_on_ambulance)
                matoshim = state_to_expand.nr_matoshim_on_ambulance
                visited_labs = set(state_to_expand.visited_labs)

                # if it is the first visit in this lab, update visited_labs and the number of matoshim on ambulance
                if lab not in state_to_expand.visited_labs:
                    visited_labs = visited_labs | {lab}
                    matoshim += lab.max_nr_matoshim

                new_state = MDAState(lab, frozenset(), frozenset(transferred), matoshim, frozenset(visited_labs))
                successors_set.append(new_state)

        # returns an successor state
        for s in successors_set:
            operator_name = ""

            if state_to_expand.nr_matoshim_on_ambulance > s.nr_matoshim_on_ambulance:   # s is the result of apartment visit
                temp_list = {}
                for a in self.problem_input.reported_apartments:
                    temp_list[a.location] = a.reporter_name
                reporter_name = temp_list[s.current_location]
                operator_name = "visit " + reporter_name
            else:   # s is the result of lab visit
                temp_list = {}
                for l in self.problem_input.laboratories:
                    temp_list[str(l.location)] = l.name
                lab_name = temp_list[str(s.current_location)]
                operator_name = "go to lab" + lab_name

            yield OperatorResult(s, self.get_operator_cost(state_to_expand, s), operator_name)
        return None
    ...
    def get_operator_cost(self, prev_state: MDAState, succ_state: MDAState) -> MDACost:
        ...
        """
        cost = self.map_distance_finder.get_map_cost_between(prev_state.current_location,\
        ...
        if cost is None:
            ...
            test_travel_temp = sum([a.nr_roommates for a in prev_state.tests_on_ambulance])
            return MDACost(distance_cost=cost, tests_travel_distance_cost=cost*test_travel_temp,\
                           optimization_objective=self.optimization_objective)
    ...
    def get_reported_apartments_waiting_to_visit(self, state: MDAState) -> Set[ApartmentWithSymptomsReport]:
        ...
        """
        return set(self.problem_input.reported_apartments) - set(state.tests_transferred_to_lab | state.tests_on_ambulance)
    ...
    def get_all_certain_junctions_in_remaining_ambulance_path(self, state: MDAState) -> List[Junction]:
        ...
        """
        remaining_junc = [a.location for a in self.get_reported_apartments_waiting_to_visit(state)] \
                         + [state.current_location]
        return sorted(remaining_junc, key=lambda elem: elem.index)
